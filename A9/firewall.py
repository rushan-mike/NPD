...
def forward(interface):
    ...
    while True:
        try:
            ...
            ip = IPv4(data[14:])
            ipsrc = str(ip.src_add)
            ipdst = str(ip.dst_add)
            ipprt = str(ip.protocol)
            hstart = ip.hlen + 14
            pad = "000000000000".encode()

            if ip.protocol == 6 or ip.protocol == "TCP":

                tcpp = TCP(data[hstart:]+pad)
                srcp = str(tcpp.src_port_flip)
                dstp = str(tcpp.dst_port_flip)

            elif ip.protocol == 17 or ip.protocol == "UDP":

                udpp = UDP(data[hstart:])
                srcp = str(udpp.src_port_flip)
                dstp = str(udpp.dst_port_flip)

            # try:
            check = database.check_against(ipsrc, ipdst, ipprt, srcp, dstp)
            # except AttributeError:
            #     check = database.check_against("n", "n", "n", "n", "n")
...
try:
    ...
    database_file = os.path.isfile('iptables.db')

    if database_file == False:
        database.create_table()
        database.insert_record("n", "n", "n", "n", "n", "permit")
